...
OUTPUT_BASE_DIR = Path("/home/jose/ONRL_ENCODER_DECODER_CAPARS/model_outputs_comparisonv2")
...
USE_OVERSAMPLING = False  # Oversample minority classes
USE_FOCAL_LOSS = True  # Focus on hard examples (reduces class imbalance impact)
FOCAL_LOSS_ALPHA = 0.25  # Weighting factor
FOCAL_LOSS_GAMMA = 2.0  # Focusing parameter

# Time-series aware augmentation (preserves temporal structure)
USE_TIME_SERIES_AUGMENTATION = True
TIME_SERIES_AUGMENTATION = {
    'vertical_shift': 0.1,  # Shift y-axis (offset)
    'horizontal_shift': 0.05,  # Small time shift
    'amplitude_scale': 0.15,  # Scale y-values
    'add_noise': 0.02,  # Gaussian noise
    'time_warp': False,  # Stretch/compress time (risky)
}
...
class MeteorologicalImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply time-series aware augmentation if enabled
    ...
    def _apply_time_series_augmentation(self, image: Image.Image) -> Image.Image:
        ...
        # Horizontal shift (small time shift)
        ...
        # Amplitude scaling (y-value scaling)
...
def train_epoch(model, dataloader, optimizer, scheduler, device, criterion=None):
    ...
    for batch in progress_bar:
        ...
        # Use custom loss if provided (e.g., FocalLoss)
